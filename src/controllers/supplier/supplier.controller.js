...
export async function getSuppliersList(req, res) {
    try {
        ...
        const createdByUuids = [
            ...new Set(supplierList.map((user) => user.createdBy).filter((uuid) => uuid)),
        ];
        ...
        const adminMap = Object.fromEntries(adminDetails.map((admin) => [admin.uuid, admin]));

        // Attach admin username to suppliers
        const suppliersWithAdmin = supplierList.map((supplier) => ({
            ...supplier,
            createdBy: supplier.createdBy ? adminMap[supplier.createdBy]?.username || supplier.createdBy : null,
        }));

        return successOkWithData(res, "Suppliers retrieved successfully.", suppliersWithAdmin);
...
export async function getSupplierDetail(req, res) {
    try {
        ...
        return successOkWithData(res, "Supplier detail retrieved successfully.", supplier);
    } catch (error) {
        return catchError(res, error);
    }
}
...
export async function updateSupplierDetail(req, res) {
    try {
        ...
        await supplier.update(fieldsToUpdateLowered, {
            where: { uuid },
        });
