...
export async function addBulkPasswords(req, res) {
    try {
        ...
        const invalidPasswords = passwords.filter(password => validatePassword(password));
        if (invalidPasswords.length > 0) {
            return validationError(res, `Invalid passwords found: ${invalidPasswords.join(", ")}. Possibel Reasons: 1. Password must be at least 8 characters long.  2. Password must contain at least one uppercase letter, one numeric digit and one special character.`);
        }
...
export async function updatePasswords(req, res) {
    try {
        ...
        const newPasswords = passwords.map(pwd => ({ password: pwd, active: true }));
